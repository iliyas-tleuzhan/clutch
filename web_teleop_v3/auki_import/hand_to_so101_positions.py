#!/usr/bin/env python3
# Upstream: https://github.com/iliyas-tleuzhan/clutch/blob/main/hand_to_so101_positions.py
# Local patch: load_config accepts FastAPI config.json (motors[].min/max) and smoothing_alpha.
...
def load_config(path):
    ...
    if "motors" in cfg:
        motors = cfg["motors"]
        if motors and isinstance(motors[0], dict) and "min" in motors[0] and "max" in motors[0]:
            limits = [[float(m["min"]), float(m["max"])] for m in motors]
        else:
            limits = limits_from_motor_config(motors)
    else:
        limits = cfg.get("motor_limits_deg", DEFAULT_LIMITS_DEG)
    if len(limits) != 6:
        raise ValueError("motor_limits_deg must contain exactly 6 [min,max] pairs.")
    alpha = float(cfg.get("smooth_alpha", cfg.get("smoothing_alpha", 0.25)))
